...
> [!danger] 🆕 ของจริงตอนนี้ **ใช้ด่านที่ 2 ตลอดเวลา** (ตรวจ 1 ส.ค. 2569)
> ตาราง `obsidian_note_chunks` มี **0 แถว** ⇒ `_chunks_available()` คืน `False` ทุกครั้ง
> ⇒ ด่านที่ 1 ไม่เคยได้ทำงานเลย ระบบไม่พังเพราะโค้ดรองรับไว้แล้วและเขียน log
> เตือนที่ `tools/obsidian.py:354` — แต่ผลค้นหยาบกว่าที่ออกแบบไว้
>
> ต่างกันตรงที่ด่าน 1 ให้**คะแนนความใกล้เคียงจริง** (0–1 จาก `similarity()`) และ
> ขยายผลตามกราฟ wikilink ได้ ส่วนด่าน 2 เป็นการจับคู่คำตรง ๆ ไม่มีคะแนน
> ⇒ คำถามที่ใช้คำไม่ตรงกับที่เขียนในโน้ตจะหาไม่เจอ ทั้งที่เนื้อหาตรงกัน
>
> ยังไม่ได้ตรวจว่าตั้งใจปิด หรือ `index_obsidian.py` ไม่ได้เติม chunk ให้ตั้งแต่แรก
> ดูรายละเอียดที่ [[04 - Data Architecture & Schema]] หัวข้อ 2.9
