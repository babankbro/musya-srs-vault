...
3. **ท่อสาย ThaiJo (งานวิจัยไทย)** — เจาะไฟล์ `thaijo_agent.run_thaijo_pipeline()`:
   ...
   - 🆕 **ด่านคัดของแปลกปลอม (Relevance Filter)** — ทำงานทันทีหลัง Fetcher ยิง event `step="relevance"`
     อ่านชื่อเรื่อง+ต้นบทคัดย่อของทุกบทความ แล้วตัดเฉพาะที่ **คนละเรื่องอย่างสิ้นเชิง** ทิ้ง
     (เคสจริง: ถามมาตรการลดหวานมันเค็ม แล้วได้ *"การรุกล้ำความเค็มในแม่น้ำท่าจีน"* ติดมา)
     รายการที่ถูกตัดจะถูกต่อท้าย `full_text` พร้อมเหตุผล ⇒ ดู [[04 - Research Tool Agents]] A0.5
...
4. **ท่อสาย PubMed (งานวิจัยสากล)** — เจาะไฟล์ `pubmed_agent.run_pubmed_pipeline()`:
   ...
   - 🆕 ผ่าน **Relevance Filter** ตัวเดียวกับฝั่ง ThaiJo — แต่ตัดสินจาก **คำถามภาษาไทยต้นทาง** ไม่ใช่ query อังกฤษที่แปลงมา
...
| `research_relevance.py` | 🆕 ยามคัดบทความคนละเรื่องออก (ใช้ร่วมทั้ง ThaiJo/PubMed) | Gemini flash-lite (temp 0) |
