...
- **ชื่อ prefix ไม่ตรงกับ `domains.py`** — โค้ดกำหนด `D2_Mental` / `D3_NCD` แต่โฟลเดอร์จริงคือ `D2_Mental Health` / `D3_NCDs` · รอดเพราะใช้ `startswith()` แต่เปราะ ถ้ามีคนสร้าง `D3_NCD_2569/` ทั้งสองจะถูกดึงมารวมกันโดยไม่มีใครรู้
