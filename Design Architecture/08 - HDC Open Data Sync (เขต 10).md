...
สร้างไฟล์หน้าตาเหมือนที่ pipeline ใช้อยู่ แล้วอัปเข้า `fileapa` พร้อม `x-amz-meta-path`
ให้ตรงโครงสร้างโฟลเดอร์เดิม · หัวตารางที่ `to_csv()` เขียนจริงเรียงตามนี้:

```text
จังหวัด, อำเภอ, ปีงบประมาณ, hospcode, areacode, รหัสอำเภอ,
<ตัวชี้วัดทีละคอลัมน์…>, รวมทุกตัวชี้วัด, date_com
```

- 3 คอลัมน์แรกต้องอยู่ตำแหน่งนี้เสมอ — `_detect_geo_keys` / `_detect_year_keys`
  ใน `multi_csv_pipeline.py` จับคู่จากชื่อคอลัมน์ ถ้าสลับที่จะเชื่อมข้อมูลข้ามไฟล์ไม่ได้
- `อำเภอ` เป็น **ชื่อไทย** · `รหัสอำเภอ` เก็บรหัส 2 หลักไว้ให้ join · `areacode` ดิบไม่แตะ
  (เพิ่ม `รหัสอำเภอ` ไว้หลัง `areacode` ไม่ใช่แทรกกลาง เพื่อไม่ให้ 3 คอลัมน์แรกขยับ)
...
> [!note] รหัสอำเภอที่แปลงไม่ได้ — แยก 2 กรณี ไม่ทิ้งแถวทั้งคู่
> เจอ 1 แถวต่อไฟล์ (ใน `220830` และ `278218`) ที่ `areacode = 33000610` คือหลักอำเภอเป็น `00`
> ซึ่งไม่ใช่รหัสอำเภอตามระบบ มท. (หน่วยบริการ `77466` ศรีสะเกษ ปี 2568)
> — **เป็นข้อมูลต้นทางที่ไม่ได้ลงรหัสอำเภอ ไม่ใช่ตารางแปลงผิด**
>
> พฤติกรรมที่ลงโค้ดจริง (`amphoe_zone10.py`) แยกสองกรณีคนละทาง:
>
> | กรณี | ผลลัพธ์ในคอลัมน์ `อำเภอ` | เตือนไหม |
> |---|---|---|
> | หลักอำเภอเป็น `00` (ต้นทางไม่ลงรหัส) | `ไม่ระบุอำเภอ` (ค่าคงที่ `UNSPECIFIED`) | ❌ ไม่เตือน — เป็นข้อบกพร่องถาวรของต้นทาง เตือนไปก็เป็นเสียงรบกวน |
> | รหัส 4 หลักอื่นที่ไม่มีในตาราง (อำเภอตั้งใหม่) | คงรหัส 2 หลักเดิมไว้ เช่น `99` | ✅ เตือนผ่าน `unknown_codes()` |
>
> เหตุที่ `00` ไม่ปล่อยเป็น `"00"`: คอลัมน์นี้ควรเป็น**ชื่อ**ล้วน การมีตัวเลขปนทำให้
> ผู้อ่านไฟล์สับสนว่าเป็นอำเภอจริงหรือไม่ · ส่วนอำเภอตั้งใหม่ต้องคงรหัสไว้ เพราะถ้าปล่อยว่าง
> แถวจะหายเงียบ ๆ จนจับไม่ได้ — `unknown_codes()` มีไว้ให้รอบรีเฟรชร้องบอกว่าตารางตกยุคแล้ว
