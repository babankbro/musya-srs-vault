...
> [!warning] ชื่อโฟลเดอร์ในคลังยาวกว่า `folder_prefix` ใน `domains.py`
> คลังใช้ `D3_NCDs` แต่ `domains.py` เขียน `D3_NCD` — ยึดตามคลัง เพราะไฟล์อยู่ตรงนั้นแล้ว
> ระบบจับคู่ด้วย **2 ตัวแรก** ของ prefix (`FOLDER_PREFIX_TO_DOMAIN` ใน `domains.py`)
> ⇒ `D3` ตรงกับ `d3` ได้แม้ชื่อเต็มต่างกัน
>
