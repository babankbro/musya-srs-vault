...
## 🆕 6.6 ด่านนำเข้าของนอก & ป้ายอธิบายข้อมูล — `app/api/hdc/*`, `app/api/datadict/*` (2026-08-01)

| ชื่อไฟล์ | วิธีเรียก | หน้าที่ |
|---|---|---|
| `hdc/preview/route.ts` | POST | ส่องตารางก่อนนำเข้า — ยังไม่เขียนอะไรลงระบบ |
| `hdc/subcatalog/route.ts` | POST | เปิดสารบัญทั้งหมวดจากลิงก์เดียว |
| `hdc/import/route.ts` | POST | นำเข้าจริง |
| `hdc/refresh/[fileId]/route.ts` | POST | ดึงใหม่ทับของเดิม · ส่งต่อ `?force=true` ให้ backend |
| `hdc/imports/route.ts` | GET | รายการที่นำเข้าแล้ว |
| `datadict/[fileId]/route.ts` | GET | พจนานุกรมของไฟล์ ให้หน้า `fileapa` แสดงข้างตัวอย่างไฟล์ |

ทั้งหมดผ่าน `requireAuth()` ตามกติกาข้อ 1 ท้ายหน้านี้ · `hdc/refresh` ต้อง**ส่งต่อ
สถานะ 409 จาก backend ให้ถึงหน้าเว็บ** ไม่ใช่กลืนเป็น error ทั่วไป เพราะ 409 คือ
"ดึงได้ไม่ครบเท่าเดิม หยุดไว้ก่อน" ที่ผู้ใช้ต้องเห็นและตัดสินใจ (ดู [[06 - API Reference — Backend]] ข้อ 12)

> [!warning] `pdf/vault/folders/route.ts` ยังไม่มี `requireAuth()` — รู้อยู่ ยังไม่แก้
> เป็นเส้นทางเดียวในหมวดนี้ที่ไม่มีด่าน และฝั่ง backend (`GET /pdf/vault/folders`)
> ก็ไม่ได้ตรวจอะไรเช่นกัน ⇒ ใครยิงเข้ามาก็ได้รายชื่อโฟลเดอร์ทั้ง vault กลับไป
>
> รั่วแค่**ชื่อโฟลเดอร์** ไม่ถึงเนื้อโน้ต ความเสี่ยงจึงต่ำ แต่ผิดแบบแผนที่ทุกเส้นทาง
> อื่นถือปฏิบัติ และชื่อโฟลเดอร์ในระบบนี้บอกใบ้โครงสร้างงานเชิงนโยบายพอสมควร
>
> แก้ได้ด้วยการเพิ่ม `requireAuth()` กับ `internalHeaders()` ให้เหมือนไฟล์ข้าง ๆ

---

## 🆕 6.7 การจัดการไฟล์ในหน้า `/fileapa` — `app/api/files/*` (2026-08-02)

| ชื่อไฟล์ | วิธีเรียก | หน้าที่ |
|---|---|---|
| `files/route.ts` | GET / POST | รายการไฟล์ทั้งหมด · อัปโหลด |
| `files/[fileId]/route.ts` | GET / PATCH / DELETE | อ่าน · **เปลี่ยนชื่อ/ย้าย** · ลบ |
| `files/folder/route.ts` | POST | 🆕 **เปลี่ยนชื่อ/ย้ายทั้งโฟลเดอร์** |

**ที่อยู่ของไฟล์ถูกเก็บไว้ 3 ที่** ที่หลุดจากกันได้ ทุกการแก้ต้องผ่าน `lib/vaultPaths.ts`:

1. `x-amz-meta-path` บน object ใน MinIO ← **AI อ่านจากตรงนี้** (`_load_path_index`)
2. `__pathdata__/{id}.json` sidecar ← หน้าเว็บอ่านจากตรงนี้
3. `csv_data_dict.vault_path` + `hdc_import.vault_path` ใน Postgres

> [!danger] กับดักที่กินเวลาสืบสวนทั้งวัน — UI บอกว่า "ไม่มีไฟล์" ทั้งที่ไฟล์อยู่ครบ
> หน้า `/fileapa` เคยแสดงข้อความ **"ยังไม่มีไฟล์ใน workspace นี้"** ในทุกกรณีที่ผิดปกติ
> ทำให้แยกไม่ออกเลยว่าเกิดอะไรขึ้น ต้นเหตุมี **3 ชั้นซ้อนกัน**
>
> **ชั้นที่ 1 — `res.ok` เชื่อไม่ได้**
> เมื่อ session หมด middleware redirect ไป `/login` แล้ว `fetch` **ตามลิงก์ให้อัตโนมัติ**
> ⇒ ได้หน้า HTML สถานะ **200** ⇒ ผ่านด่าน `if (!res.ok) throw` ⇒ ไปพังตอน `res.json()`
> เป็น `SyntaxError: Unexpected token '<'` ที่ไม่มีใครอ่านออก
> **แก้:** ตรวจ `content-type` และ `res.redirected` ด้วย → โยน `SessionExpiredError`
>
> **ชั้นที่ 2 — catch แล้วลง console เฉย ๆ**
> `loadFiles()` จับ error แล้ว `console.error` อย่างเดียว ⇒ `items` ว่าง ⇒ หน้าเว็บ
> แสดงข้อความ "ไม่มีไฟล์" ⇒ **ผู้ใช้เข้าใจว่าข้อมูลหาย**
> **แก้:** เก็บ error ไว้ใน state แล้วแสดง แยก "โหลดไม่สำเร็จ" ออกจาก "ไม่มีไฟล์"
>
> **ชั้นที่ 3 — ไม่มีสถานะกำลังโหลด**
> `GET /api/files` ใช้เวลา **7–8 วินาที** ที่ 317 ไฟล์ (ยิง `statObject` รายไฟล์ + เดิม
> ยิง `getObject` หา APA sidecar อีกไฟล์ละครั้งซึ่งส่วนใหญ่ไม่มีจริง)
> ระหว่างรอ หน้าเว็บขึ้นว่า "ไม่มีไฟล์" — **นี่คืออาการที่ผู้ใช้เจอจริงบ่อยที่สุด**
> **แก้:** เพิ่ม spinner "กำลังโหลดรายการไฟล์…" และกวาด prefix `__meta__/` ครั้งเดียว
> แทนการยิงรายไฟล์
>
> **บทเรียน:** เมื่อ "ไม่มีข้อมูล" กับ "ยังโหลดไม่เสร็จ" กับ "โหลดไม่สำเร็จ" แสดงผล
> เหมือนกัน จะ debug ไม่ได้เลย — และผู้ใช้จะสรุปเองว่าข้อมูลหาย ซึ่งน่ากลัวกว่า
> การขึ้น error ตรง ๆ มาก

**ค้นหาไฟล์** — ช่องค้นหาใน Explorer ค้นทั้งชื่อและเส้นทาง แยกคำด้วยช่องว่างแล้ว
ต้องเจอครบทุกคำ (พิมพ์ `ไต 2568` ได้เฉพาะที่มีทั้งสอง) และกางโฟลเดอร์อัตโนมัติ —
ผลค้นหาที่ซ่อนอยู่ในโฟลเดอร์ที่ยังพับอยู่เท่ากับหาไม่เจอ
